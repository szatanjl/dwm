...
enum { SchemeNorm, SchemeSel, SchemeNormI, SchemeSelI }; /* color schemes */
...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, ismax;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
drawbar(Monitor *m)
{
	...
	Clr *scmnorm = scheme[m == selmon ? SchemeNorm : SchemeNormI];
	Clr *scmsel = scheme[m == selmon ? SchemeSel : SchemeSelI];
	...
	if (systray && m == selmon) {
		...
		XSetForeground(dpy, drw->gc, scmnorm[ColBg].pixel);
		XFillRectangle(dpy, systray->win, drw->gc, 0, 0, stw, bh);
		XMapWindow(dpy, systray->win);

		x = lrpad / 2;
		for (c = systray->icons; c; c = c->next) {
			XSetWindowAttributes wa = { .background_pixel = scmnorm[ColBg].pixel };
...
movestack(const Arg *arg)
{
	...
	if (arg->i > 0) {
		...
		if (!c)
			for(c = cl->clients; c && (!ISVISIBLE(c, selmon) || c->isfloating); c = c->next);
	} else {
	...
	for (i = cl->clients; i && (!p || !pc); i = i->next) {
	...
	/* swap c and selmon->sel cl->clients in the cl->clients list */
	if (c && c != selmon->sel) {
		...
		if (selmon->sel == cl->clients)
			cl->clients = c;
		else if (c == cl->clients)
			cl->clients = selmon->sel;
...
resetcfact(const Arg *arg)
{
	...
	for(c = cl->clients; c != NULL; c = c->next)
		if (ISVISIBLE(c, selmon) && !c->isfloating)
			c->cfact = 1.0;
...
setup(void)
{
	...
	/* init workspaces */
	workspaces = ecalloc(LENGTH(tags), sizeof(Workspace));
	for (i = 0; i < LENGTH(tags); i++) {
		workspaces[i].sellt = 0;
...
tile(Monitor *m)
{
	...
	for (i = 0, my = ty = gappx, c = nexttiled(cl->clients, m); c; c = nexttiled(c->next, m), i++)
		if (i < m->nmaster) {
